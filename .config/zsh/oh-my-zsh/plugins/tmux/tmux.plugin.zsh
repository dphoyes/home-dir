...
if which tmux &> /dev/null
	...
	# Automatically connect to a previous detached session if it exists
	[[ -n "$ZSH_TMUX_AUTOCONNECT_DETACHED" ]] || ZSH_TMUX_AUTOCONNECT_DETACHED=true
	...
	function _zsh_tmux_plugin_run()
	{
		...
		elif [[ "$ZSH_TMUX_AUTOCONNECT_DETACHED" == "true" ]]
		then
			prev_session=`tmux ls | grep -v attached | tail -n1 | cut -d: -f1`

			if [ -n "$prev_session" ]; then
				\tmux `[[ "$ZSH_TMUX_ITERM2" == "true" ]] && echo '-CC '` attach -t $prev_session
			else
